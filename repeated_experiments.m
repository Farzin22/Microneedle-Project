%repeated experiments analysis matlab code
%author Farzin Akbar
...
title(str_DT,'interpreter','latex');


%%%%%%%%%%%%%%%%%%%%%%%saving the figure %%%%%%%%%%%%%
cd(path);
file_name =strcat(path,'\',repetitive_index,'_','analyzed')
print(g,strcat(repetitive_index,'_analyzed'),'-dpng', '-r300')
print(g,strcat(repetitive_index,'_analyzed'),'-dtiff')
saveas(g, strcat(repetitive_index,'_analyzed')) % save figure
...
if(a==0)
	...
	fprintf(fid, 'Number of Microneedles ; Tip Diameter (µm); Penetration Force (mN); Force Drop (mN); Displacement at Penetration(µm);Time at Penetration (s);Threshold Force;FD_movingaverage;FT_movingaverage;DT_movingaverage;VD_movingaverage');
